...
**Table shape:** 383 rows × 80 columns (days with all 3 core sources present)
...
|---|---|---|---|---|---|
| `subset_energy_j` | float64 | Joules | 1.1e9 – 4.5e10 | 0% | Total daily energy from all 6 inverters (∑ power × 600s). |
...
| `inverter_records` | float64 | count | 43 – 144 | 0% | Number of 10-min records received. Expected: 144 (24h × 6). |
...
| Column | Type | Unit | Range | Nulls | Description |
|---|---|---|---|---|---|
| `irradiance_horizontal_sum` | float64 | W/m² (daily sum) | 0 – 413k | 0% | Global Horizontal Irradiance daily total. |
| `irradiance_tilted_sum` | float64 | W/m² (daily sum) | 0 – 434k | 0% | Tilted (plane-of-array) irradiance daily total. **Primary irradiance metric.** |
| `irradiance_records` | float64 | count | 4 – 96 | 0% | 15-min records received. Expected: 96 (24h × 4). |
...
| Column | Type | Unit | Range | Nulls | Description |
|---|---|---|---|---|---|
| `pm25_mean` | float64 | µg/m³ | 3.2 – 47.7 | 0% | Daily mean PM2.5 concentration. |
| `pm25_max` | float64 | µg/m³ | 4.4 – 72.9 | 0% | Daily max PM2.5. |
| `pm10_mean` | float64 | µg/m³ | 4.4 – 138.5 | 0% | Daily mean PM10 (coarse dust). |
| `pm10_max` | float64 | µg/m³ | 5.8 – 261.4 | 0% | Daily max PM10. |
...
> [!TIP]
> **Best soiling predictors** from literature: PM10 (deposition), precipitation (cleaning), humidity×PM interaction (cementation), and days-since-last-rain (accumulation proxy).
...
| Column | Type | Unit | Range | Nulls | Description |
|---|---|---|---|---|---|
| `solcast_ghi_sum` | float64 | J/m²/day | 2.9M – 26.3M | 0% | Global Horizontal Irradiance (satellite). |
| `solcast_gti_sum` | float64 | J/m²/day | 2.8M – 28.1M | 0% | Global Tilted Irradiance (satellite). Compare with ground-measured `irradiance_tilted_sum`. |
| `solcast_dni_mean` | float64 | W/m² | 0 – 369.5 | 0% | Direct Normal Irradiance daily mean. |
...
| `performance_loss_pct_proxy` | float64 | % | 0 – 80.9 | 13.8% | `100 × (1 − normalized_output / rolling_clean_baseline)`. All-cause loss including soiling. Clipped [0, 100]. |
...
Same calculations as [§12](#12-combined-performance-metrics), but using **Tier-1 (B2) energy only**.
